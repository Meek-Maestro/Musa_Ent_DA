...
import { CustomerAccounting } from "@renderer/interface";
...
import CustomerAccountingPrint from "./printouts/CustomerAcc";

interface props {
    close: () => void
}

interface formProps { period: string, end_date: string, start_date: string, customer_id?: number }

export default observer(function CustomerAccouting({ close }: props) {
    const { loading, loadCustomerAccountingPayload } = reportPayload()
    ...
    useEffect(() => {
        setCustomerAccounting(reportsLoader.customerAccounting || [])
    }, [reportsLoader.customerAccounting])
    ...
    return (
        <>
            <Stack>
                ...
                <Title order={2}>Customer Accounting Report</Title>

                <form
                    onSubmit={form.onSubmit(async () => {
                        await loadCustomerAccountingPayload(form.values);
                    })}
                >
                    ...
                    >
                        ...
                        <Table striped highlightOnHover className={classes.table}>
                            ...
                            <Table.Tbody className={classes.scrollableBody}>
                                {customerAccounting.map((item, index) => (
                ...
                <div style={{ display: "none" }}>
                    <CustomerAccountingPrint ref={printRef} customerAccounting={customerAccounting} />
                </div>
            </Stack>
        </>
    )
