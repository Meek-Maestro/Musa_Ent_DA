...
import SupplierPrint from "./printouts/Supplier";
...
export default observer(function SuppliersRep({ close }: props) {
    const { loading, loadSuppliersByPayload } = reportPayload()
    ...
    useEffect(() => {
        setSuppliers(reportsLoader.supplier || [])
    }, [reportsLoader.supplier])
    return (
        <>
            <Stack>
                <Group>
                ...
                <Title order={2}>Supplier Report</Title>

                <form
                    onSubmit={form.onSubmit(async () => {
                        await loadSuppliersByPayload(form.values);
                    })}
                >
                ...
                <div style={{ display: "none" }}>
                    <SupplierPrint ref={printRef} supplier={suppliers} />
                </div>
            </Stack>
        </>
    )
