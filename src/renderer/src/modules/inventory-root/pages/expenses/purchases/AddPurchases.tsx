...
export default function AddPurchase() {
  const { purchase_form, addPurchase, submitting } = usePurchase();
  const [suppliers, setSupplier] = useState<any[]>([]);
  const [items, setItems] = useState<any[]>([{ product_name: "", quantity: 0, unit_price: 0, discount: 0, total: 0 }]);
  ...
  const handleAddItem = () => {
    setItems([...items, { product_name: "", quantity: 0, unit_price: 0, discount: 0, total: 0 }]);
  };
  ...
  const handleItemChange = (index: number, field: string, value: string | number) => {
    ...
    newItems[index][field] = field === "quantity" || field === "unit_price" || field === "discount" || field === "total" ? parseFloat(value as string) : value;
    setItems(newItems);
    purchase_form.setFieldValue("purchase_items", newItems);
  };
    ...
    <AppPageWrapper title="New Purchase" right={<UserButton />}>
      ...
      <form
        onSubmit={purchase_form.onSubmit(async () => {
          ...
          if (await addPurchase(formData)) {
            purchase_form.reset();
            setItems([{ product_name: "", quantity: 0, unit_price: 0, discount: 0, total: 0 }]);
            setDamagedItems([]);
          }
        })}
      >
